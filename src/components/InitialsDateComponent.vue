...
const updateDateAndDay = () => {
    ...
    currentDay.value = date.toLocaleDateString(getCurrentInstance()?.appContext.config?.global?.locale, { weekday: 'long' });
    currentMonth.value = date.toLocaleDateString(getCurrentInstance()?.appContext.config?.global?.locale, { month: 'long' });
};
